...
# ПРОКСИ (твои находки!)
PROXY_CONFIG = {
    "austria": "socks5h://152.53.137.180:1081",
    "belgium": "http://35.205.61.211:3128",
}

current_source = "servustv"
current_proxy = "austria"
...
# ========== ФУНКЦИИ ==========
def get_streamlink_session(proxy_key=None):
    session = streamlink.Streamlink()
    session.set_option("http-header", {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36"
    })
    if proxy_key and proxy_key in PROXY_CONFIG:
        session.set_option("http-proxy", PROXY_CONFIG[proxy_key])
    return session

def fetch_via_streamlink(url, proxy_key=None):
    try:
        session = get_streamlink_session(proxy_key)
...
async def get_stream():
    global manual_stream_url, current_source, current_proxy
    ...
    stream_url = fetch_via_streamlink(url, current_proxy)
    if stream_url:
        return {"m3u8": stream_url, "provider": f"{current_source.upper()} ({current_proxy})", "status": "ok"}
...
async def commentator_update(data: dict):
    global current_source, current_proxy
    if data.get("source") in SOURCES:
        current_source = data["source"]
    if data.get("proxy") in PROXY_CONFIG:
        current_proxy = data["proxy"]
    return {"status": "ok", "source": current_source, "proxy": current_proxy}
